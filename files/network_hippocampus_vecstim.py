...
class Network:
    ...
    def set_pyramidal_topology(self, type):
        ...
        if type == "circular":
            ...
            self.n_pyr_ca3 = 800
            if self.create_ca1_network:
                self.pyr_ca1_x = 1e9+ np.concatenate([np.arange(0,40,1)*dx for i in range(20)])-1000.0
    ...
    def create_populations(self):
        ...
        self.pyr_ca3 = Population(cell_type=PyrAdr_CA3, pgidlist=gid_pyr_ca3, n=self.n_pyr_ca3, x= 0, y=0, z=0, dx=50.0, amp= 40e-3, dur=1e9, delay=2*h.dt, nseg = self.nseg_pyr, naux = 0, pyr=True, x_position=self.pyr_ca3_x,y_position=self.pyr_ca3_y, record_mp = self.record_mp) # Estas corrientes se han anadido
        self.bas_ca3 = Population(cell_type=Bwb,        pgidlist=gid_bas_ca3, n=self.n_bas_ca3, x=10, y=0, z=0, dx=50.0, amp= 0,     dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[0],record_mp = self.record_mp) # para obtener el baseline state
        self.olm_ca3 = Population(cell_type=Ow,         pgidlist=gid_olm_ca3, n=self.n_olm_ca3, x=20, y=0, z=0, dx=50.0, amp=-25e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[1],record_mp = self.record_mp) # dur tiene que se mayor o igual que el tiempo de simulacion
        self.cell_ca3 = self.pyr_ca3.cell+self.bas_ca3.cell+self.olm_ca3.cell
        self.pop_ca3  = [self.pyr_ca3, self.bas_ca3, self.olm_ca3]

        if self.create_ca1_network:
            self.pyr_ca1 = Population(cell_type=PyrAdr_CA1, pgidlist=gid_pyr_ca1, n=self.n_pyr_ca1, x=1e3+0,  y=0, z=0, dx=50, amp= 40e-3, dur=1e9, delay=2*h.dt, nseg = self.nseg_pyr, naux=self.n_pop[2],record_mp = self.record_mp) # Estas corrientes se han anadido
            self.bas_ca1 = Population(cell_type=Bwb,        pgidlist=gid_bas_ca1, n=self.n_bas_ca1, x=1e3+10, y=0, z=0, dx=50, amp= 0,     dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[3],record_mp = self.record_mp) # para obtener el baseline state
            self.olm_ca1 = Population(cell_type=Ow,         pgidlist=gid_olm_ca1, n=self.n_olm_ca1, x=1e3+20, y=0, z=0, dx=50, amp=-25e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[4],record_mp = self.record_mp) # dur tiene que se mayor o igual que el tiempo de simulacion
            self.cck_ca1 = Population(cell_type=Cck_cell,   pgidlist=gid_cck_ca1, n=self.n_cck_ca1, x=1e3+30, y=0, z=0, dx=50, amp= 10e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[5],record_mp = self.record_mp) # dur tiene que se mayor o igual que el tiempo de simulacion
    ...
    def set_delays_to_synpatic_connections(self,sigma, seed):
        ...
        for key in self.delays_neurons_neurons.keys():
            ...
            if self.create_ca1_network & key.endswith("ca1"):
                ...
                delay_mean = self.delay_neurons_neurons[key][0]
                for n in nsyn:
                    self.delays_neurons_neurons[key].append( np.abs(delay_mean+sigma*np.random.normal(0,self.jitter,(nsyn,po.n))) ) 
                    seed += 1
        return seed
    ...
    def make_all_CellStims(self,simdur):
        ...
        self.tvec["dg_burst"]  = tvec_dg_burst
        self.idvec["dg_burst"] = idvec_dg_burst
        ...
        print("Creating dictionary for dg burst inputs:")
        print("----------------------------------------")

        keys1 = ["dg_burst_to_pyr_ca3", "dg_burst_to_bas_ca3"]
        self.external_inputs_data = dict.fromkeys(keys1)
        keys2 = ["idv","spikes","population","synapses","delays","weights","connectivity" ]
        for key in keys1:
            self.external_inputs_data[key] = dict.fromkeys(keys2)
