...
class Network:
    ...
    def create_populations(self):
        ...
        #print("0)Host: "+str(pc.id())+". len_pyr "+str(len(gid_pyr_ca1))+" "+str(gid_pyr_ca1))
        #print("0)Host: "+str(pc.id())+". len_bas "+str(len(gid_bas_ca1))+" "+str(gid_bas_ca1))
        #rint("0)Host: "+str(pc.id())+". len_olm "+str(len(gid_olm_ca1))+" "+str(gid_olm_ca1))
        #print("0)Host: "+str(pc.id())+". len_cck "+str(len(gid_cck_ca1))+" "+str(gid_cck_ca1))
        ...
        self.cck_ca1 = Population(cell_type=Cck_cell,   pgidlist=gid_cck_ca1, n=self.n_cck_ca1, x=30, y=0, z=0, dx=50, amp= 62.5e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[2], record_mp=self.record_mp) # dur tiene que se mayor o igual que el tiempo de simulacion
        ...
        for key in self.delays_inputs_neurons.keys():
            if key.endswith("ca1"):
                ...
                for n in nsyn:
                    ...
                    #self.delays_inputs_neurons[key].append( delay_mean+np.zeros((n,po.n)))
                    seed += 1
        return seed
        ...
        for key in self.cell_labels:
            if key.endswith("ca1"):
                ...
                for k in keys:
                    ...
                    #self.delays_noise_neurons[key][k] = delay_mean+np.zeros(po.n)
                seed+=1
        return seed
            ...
            for post_id, all_pre in enumerate(conn_):
                for j, pre_id in enumerate(all_pre):
                    for syn,w in zip(syn_list[k],w_list[k]):
                        post_cell = pc.gid2cell(trg.naux+post_id)
                        crush
                        self.ncl_.append(h.NetCon(ns_src[pre_id], post_cell.__dict__[syn].syn, 0, delay[j,post_id], w))
    ...
    def set_connectivity(self, nsrc, ntrg, conv):
        ...
        #print("0.5)Host: "+str(pc.id())+str(conn.shape))
        conn_ = conn[ np.arange(pc.id(), ntrg, pc.nhost()),:]
        #print("0.8)Host: "+str(pc.id())+str(conn_.shape))
        return conn_
    ...
    def set_cells_connectivity(self):
        ...
        for key in keys:
            if key.endswith("ca1"):
                ...
                #print("1)Host:"+str(pc.id())+" " + key+" connections: ("+str(len(self.nsyns_neurons_neurons[key]))+" "+str(self.connectivity_neurons_neurons[key][-1].shape))
    ...
    def f_conns(self):
        ...
        for key in self.weights_neurons_neurons.keys():
            if key.endswith("ca1"):
                ...
                pc.barrier()
                print("2) Host"+str(pc.id())+" " +key, syn,str(conn[0].shape) )
            ...
            for post_id, all_pre in enumerate(conn_):
                for j, pre_id in enumerate(all_pre):
                    ...
                    for syn,w in zip(syn_list[k],w_list[k]):
                        ...
                        #try:
                            #if("CCK-expressing cell" in str(src.cell[0])):
                            #    print(str(pc.id())+" from "+str(pre_id+src.naux) +" to " + str(post_id))
                        trg.nc.append(pc.gid_connect(pre_id+src.naux, trg.cell[post_id].__dict__[syn].syn))
                        trg.nc[-1].weight[0] = w
                        #trg.nc[-1].delay = delay[k][j,post_id]
                        trg.nc[-1].delay = delay[k][j,trg.cell[post_id].id-trg.naux]
                        trg.nc[-1].threshold = 0.0
                        print("Delay between cell ="+str(pre_id+src.naux)+" and "+str(trg.cell[post_id])+" is "+str(delay[k][j,trg.cell[post_id].id-trg.naux]))
                        #except:
                            #raise Exception("HERE")
                            #raise Exception("3)Host"+str(pc.id())+" "+str(src.cell[0])+"->"+str(trg.cell[0])+"  " + str(pre_id+src.naux)+"->"+str(post_id)+"  ")
    ...
    def load_all_CellStims(self,simdur):
        ...
        print("CA3 file "+str(os.path.join(burst_folder,file)))
        data = file_management.load_lzma(os.path.join(burst_folder,file))
        #print(data["ca3"]["pyr"].keys())
        x,y = data["tvec"], data["idvec"]
