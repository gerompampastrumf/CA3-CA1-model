...
class Network:
    ...
    def create_populations(self):
        ...
        self.pyr_ca3 = Population(cell_type=PyrAdr_CA3, pgidlist=gid_pyr_ca3, n=self.n_pyr_ca3, x= 0, y=0, z=0, dx=50.0, amp= 40e-3, dur=1e9, delay=2*h.dt, nseg = self.nseg_pyr, naux = 0, pyr=True, x_position=self.pyr_ca3_x,y_position=self.pyr_ca3_y, record_mp = self.record_mp, resolution=self.resolution) # Estas corrientes se han anadido
        self.bas_ca3 = Population(cell_type=Bwb,        pgidlist=gid_bas_ca3, n=self.n_bas_ca3, x=10, y=0, z=0, dx=50.0, amp= 0,     dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[0],record_mp = self.record_mp, resolution=self.resolution) # para obtener el baseline state
        self.olm_ca3 = Population(cell_type=Ow,         pgidlist=gid_olm_ca3, n=self.n_olm_ca3, x=20, y=0, z=0, dx=50.0, amp=-25e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[1],record_mp = self.record_mp, resolution=self.resolution) # dur tiene que se mayor o igual que el tiempo de simulacion
        self.cell_ca3 = self.pyr_ca3.cell+self.bas_ca3.cell+self.olm_ca3.cell

        if self.create_ca1_network:
            self.pyr_ca1 = Population(cell_type=PyrAdr_CA1, pgidlist=gid_pyr_ca1, n=self.n_pyr_ca1, x=1e3+0,  y=0, z=0, dx=50, amp= 40e-3, dur=1e9, delay=2*h.dt, nseg = self.nseg_pyr, naux=self.n_pop[2],record_mp = self.record_mp, resolution=self.resolution) # Estas corrientes se han anadido
            self.bas_ca1 = Population(cell_type=Bwb,        pgidlist=gid_bas_ca1, n=self.n_bas_ca1, x=1e3+10, y=0, z=0, dx=50, amp= 0,     dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[3],record_mp = self.record_mp, resolution=self.resolution) # para obtener el baseline state
            self.olm_ca1 = Population(cell_type=Ow,         pgidlist=gid_olm_ca1, n=self.n_olm_ca1, x=1e3+20, y=0, z=0, dx=50, amp=-25e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[4],record_mp = self.record_mp, resolution=self.resolution) # dur tiene que se mayor o igual que el tiempo de simulacion
            self.cck_ca1 = Population(cell_type=Cck_cell,   pgidlist=gid_cck_ca1, n=self.n_cck_ca1, x=1e3+30, y=0, z=0, dx=50, amp= 10e-3, dur=1e9, delay=2*h.dt, nseg = 1, naux=self.n_pop[5],record_mp = self.record_mp, resolution=self.resolution) # dur tiene que se mayor o igual que el tiempo de simulacion
