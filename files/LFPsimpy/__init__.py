...
class LfpElectrode:
    ...
    def __init__(self,
                 ...
                 neuron_type = "Pyramidal CA3", 
                 section_list = ["soma", "Bdend", "Adend1", "Adend2", "Adend3"]):
        ...
        self.nsections = len(self.section_list)
        self.neuron_type = neuron_type
        self.values = []
        self.values_per_section = []
        self.times = []
    ...
    def setup_neuron_plot_vars(self):
        ...
        #dir = "C:/Users/Asus/Google Drive/hippocampus/hippocampus_creta/rebuild/"

        #dir = "C:/Users/Jaime/Google Drive/hippocampus/hippocampus_creta/rebuild/"
    ...
    def insert(self):
        ...
        for sec in h.allsec():
            if str(sec).__contains__(self.neuron_type):
    ...
    def compute(self):
        ...
        # e.g. extremelly large   return 0
        if self.h.t == 0:
            return 0, np.zeros(self.nsections)

        #result_per_section_dict = {} 
        #for sec in self.section_lfps.keys():
        #   result_per_section_dict[sec] = self.section_lfps[sec].compute()
                               
        # Sum the LFPs of each section
        result_per_section = np.array( [ sec_lfp.compute() for sec_lfp in self.section_lfps.values() ] )
        result = sum( result_per_section )
        
        result_per_section_list = [] 
        result_size = len(result_per_section)
        
        #for i in range( result_size//self.nsections): 
        #    result_per_section_list.append( result_per_section[self.nsections*i:self.nsections*(i+1)] )
          
        result_per_section = np.sum( result_per_section.reshape( result_size//self.nsections, self.nsections ), axis = 0)
        ...
        # return result, result_per_section
        return result, result_per_section#dict

    def collect(self):
        h = self.h
        h.dt = 0.1 # added on 07/06/2022
        if h.t > h.tstop:
        ...
        time = self.h.t
        value, value_per_section = self.compute()
        #value = self.compute()

        self.times.append(time)
        self.values.append(value)
        self.values_per_section.append(value_per_section)
