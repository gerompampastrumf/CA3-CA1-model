...
class Cell:
    "General cell"
    def __init__(self,x,y,z,id, nseg, record): # new variable, record
        self.x=x
        self.y=y
        self.z=z
        self.id=id
...
class Bwb(Cell):
    ...
    def set_synapses(self):
        self.somaAMPAf  = Synapse(sect=self.soma, loc=0.5, tau1=0.05, tau2=5.3,  e=0)   # background
        self.somaGABAf  = Synapse(sect=self.soma, loc=0.5, tau1=0.07, tau2=9.1,  e=-80) # backgorund
...
class Ow(Cell):
    ...
    def set_synapses(self):
        ...
        self.somaAMPAf  = Synapse(    sect=self.soma, loc=0.5, tau1=0.05, tau2=5.3, e=0.0)
        #self.somaGABAf.tau  = 2
        #self.somaGABAf.e = -80
        #self.somaAMPAf.tau = 2
        #self.somaAMPAf.e = 0
...
class Cck(Cell):
    ...
    def set_conductances(self):
        self.soma.Ra = 150
        self.soma.insert('iconc_Ca') #read cao and cai
        self.soma(0.5).iconc_Ca.catau = 10
        self.soma.cao = 2
        self.soma.cai = 5e-6

        self.soma.insert('ch_CavL')    # READ eca
        self.soma.insert('ch_CavN')    # READ eca
        self.soma.insert('ch_HCN')     # READ e (according to the paper -30mV)
        self.soma.insert('ch_KCaS')    # READ eca and cai (calcim concentration dynamics)
        self.soma.insert('ch_Kdrfast') # READ ek
        self.soma.insert('ch_KvA')     # READ ek
        self.soma.insert('ch_KvCaB')   # READ ek and cai
        self.soma.insert('ch_KvGroup') # READ ek
        self.soma.insert('ch_Navcck')  # READ ena
        self.soma.insert('ch_leak')    # READ e
        ...
        self.soma(0.5).ch_CavL.gmax    = 0.0027   #eca
        self.soma(0.5).ch_CavN.gmax    = 0.00002  #eca
        self.soma(0.5).ch_HCN.gmax     = 0.0001   #e
        self.soma(0.5).ch_KCaS.gmax    = 0.000004 # READ eca and cai (calcim concentration dynamics)
        self.soma(0.5).ch_Kdrfast.gmax = 0.000008 #ek
        self.soma(0.5).ch_KvA.gmax     = 0.0004
        self.soma(0.5).ch_KvCaB.gmax   = 0.00004
        self.soma(0.5).ch_KvGroup.gmax = 0.0026
        self.soma(0.5).ch_Navcck.gmax  = 0.018
        self.soma(0.5).ch_leak.gmax    = 1./Rmsoma

        self.soma(0.5).ch_HCN.e = eHCNval
        self.soma(0.5).ch_leak.e = eleakval
        self.soma.ek  = ekval
        self.soma.ena = enaval
        self.soma.eca = ecaval
